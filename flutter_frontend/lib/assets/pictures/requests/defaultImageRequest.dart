...
class DefaultImageRequest extends StatefulWidget {
  final String pictureId;

  const DefaultImageRequest({Key? key, required this.pictureId})
    : super(key: key);

  @override
  State<DefaultImageRequest> createState() => _DefaultImageRequest();
}

class _DefaultImageRequest extends State<DefaultImageRequest> {
  ...
  Future<void> _fetchPictures() async {
    try {
      ...
      final pictureResponse = await http.get(
        Uri.parse('$server/get/picture/$albumSerial'),
        headers: {
          'Authorization': token ?? '',
