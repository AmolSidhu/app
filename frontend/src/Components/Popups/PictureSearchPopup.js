...
const PictureSearchPopup = () => {
  const [searchRows, setSearchRows] = useState([
    { query: "", searchType: "Tags", matchType: "Exact Match" },
  ]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const addSearchRow = () => {
    setSearchRows([...searchRows, { query: "", searchType: "Tags", matchType: "Exact Match" }]);
  };
  ...
  const handleSearch = async () => {
    ...
    setError("");
    try {
      const response = await fetch(`${server}/generate/picture_query/`, {
        method: "POST",
        headers: {
      ...
      if (response.status === 201) {
        ...
        window.location.href = `/picture/search/?searchId=${searchId}`;
      } else {
...
export default PictureSearchPopup;
