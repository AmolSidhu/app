    ...
    useEffect(() => {
        const fetchPictures = async () => {
            try {
                ...
                const query = urlParams.get('searchId');

                if (!query) {
                    setError('Error in fetching Query');
                    return;
                }

                const pictureResponse = await fetch(`${server}/fetch/picture_query/${query}/`, {
                    method: 'GET',
                    headers: {
                        'Authorization': token,
...
export default PictureSearchRequest;
